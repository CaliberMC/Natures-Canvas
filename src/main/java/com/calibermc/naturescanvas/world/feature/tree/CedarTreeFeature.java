...
public class CedarTreeFeature extends NCTreeFeature<EvergreenTreeConfiguration> {
    public CedarTreeFeature(Codec<EvergreenTreeConfiguration> codec) {
        super(codec);
    }
...
//        int baseHeight = GeneratorUtil.nextIntBetween(random, (int) (height * 0.6F), (int) (height * 0.4F)); //original
        int baseHeight = GeneratorUtil.nextIntBetween(random, (int) (height * 0.1F), (int) (height * 0.2F)); //original
...
//            int radius = Math.min(Math.min((i + 3) / 3, 2 + ((leavesHeight - i) * 2)), 6);//too wide
//            int radius = Math.min(Math.min((i + 3) / 3, 2 + ((leavesHeight - i) / 2)), 4); // Use
//            int radius = Math.min(Math.min((i + 2) / 4, 2 + (leavesHeight - i)), 4); // original

            double leavesTaper;

            int radius;
            if (i < 2) {
                // For the first 2 blocks, interpolate from 0 to 1
                leavesTaper = (double) i / 2;
            } else {
                // For the remaining blocks, interpolate from 1 to 5
...
//                    if (dist > 0 && y > 6 && y < (baseHeight - 2) && random.nextInt(10) == 0) { //more branches
                        double theta;
                        if (x == 0 && z == 0) {
                        ...
                        // branch length 1 - 4 blocks
                        int branchLength = (3 - dist) + 1 + random.nextInt(3);
