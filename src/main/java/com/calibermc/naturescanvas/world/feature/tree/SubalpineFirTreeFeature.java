...
public class SubalpineFirTreeFeature extends NCTreeFeature<EvergreenTreeConfiguration> {
    public SubalpineFirTreeFeature(Codec<EvergreenTreeConfiguration> codec) {
        super(codec);
    }
    ...
    protected boolean doPlace(@NotNull WorldGenLevel world, RandomSource random, @NotNull BlockPos startPos, BiConsumer<BlockPos, BlockState> roots, BiConsumer<BlockPos, BlockState> logs, FoliagePlacer.FoliageSetter leaves, TreeConfiguration configBase) {
        ...
        for (int i = 0; i < leavesHeight; i++) {
            ...
            int radius = getLeavesRadius(height, i, leavesHeight);
    ...
    private static int getLeavesRadius(int height, int i, int leavesHeight) {
        double leavesTaper;
        double multiplier;
        int radius;
        ...
        if (height < 15) {
            ...
            // For trees with height in range 15 to 30 inclusive
            multiplier = 2;
        } else {
            // For trees with height equal to or greater than 31
            multiplier = 4;
        }
